...
export default function DashboardPage() {
  ...
  return <section className={styles.page}>
    <header className={styles.header}><div><span className="eyebrow">Schweppes Admin / Overview</span><h1>Good morning.</h1><p>Your store at a glance, from sales momentum to bottles on hand.</p></div><Link href="/dashboard/products" className={styles.primary}>Manage catalogue <span aria-hidden="true">-&gt;</span></Link></header>
    ...
    <div className={styles.columns}><section className={styles.panel}><div className={styles.panelHead}><div><span className="eyebrow">Activity</span><h2>Recent orders</h2></div><Link href="/dashboard/orders">View all</Link></div>{stats.recentOrders.length ? stats.recentOrders.map((order) => <div className={styles.row} key={order._id}><div><strong>#{order._id.slice(-7).toUpperCase()}</strong><small>{order.items?.length || 0} item(s)</small></div><b>${Number(order.totalAmount || 0).toFixed(2)}</b><span className={`${styles.badge} ${styles[order.status || "pending"] || ""}`}>{order.status || "pending"}</span></div>) : <p className={styles.empty}>{loading ? "Loading activity..." : "No orders yet."}</p>}</section><section className={styles.panel}><div className={styles.panelHead}><div><span className="eyebrow">Best sellers</span><h2>Moving fastest</h2></div><Link href="/dashboard/products">Catalogue</Link></div>{stats.topProducts.length ? stats.topProducts.map((product) => <div className={styles.row} key={product._id}><div><strong>{product._id}</strong><small>{product.units} units sold</small></div><b>${Number(product.revenue || 0).toFixed(2)}</b></div>) : <p className={styles.empty}>Sales will appear here after your first order.</p>}</section></div>
  </section>;
}